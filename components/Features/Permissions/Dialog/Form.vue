...
import { formPermissionSchema } from '@/app/validations/permission';
...
const form = useForm({
  validationSchema: formPermissionSchema,
})
    ...
    <UiDialogContent class="max-w-lg">
      ...
      <form @submit="onSubmit">
        <div class="space-y-3">
          ...
          <UiFormField v-slot="{ componentField }" name="method" :value="item?.method">
            <UiFormItem>
              <UiFormLabel>Method</UiFormLabel>
              <UiFormControl>
                <UiInput type="text" placeholder="method" v-bind="componentField" />
              </UiFormControl>
              <UiFormMessage />
            </UiFormItem>
          </UiFormField>
